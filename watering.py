...
try:
    while True:
        #LED debugging bs
        GPIO.output(7,True)
        #Turn on sensor & camera and sleep a min
        camera.start_preview()
        GPIO.output(38,True)
        time.sleep(5)
        #If soil is dry open valve for water(ing) time
        if (GPIO.input(29))==1:
            GPIO.output(37,True)
            time.sleep(water)
        ...
        camera.capture('/home/pi/WeedPlants/pic_%03d.jpg' % (pic_num))
        camera.stop_preview()
        pic_num = pic_num + 1
        GPIO.output(7,False)
        
        #Wait for interval period, flashing debug LED every 30 seconds
        count_AA = 0
